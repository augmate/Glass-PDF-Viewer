...
public class FoxitDoc{
	...
	//protected int pageHandles[currentIndex] = 0;
	protected int[] pageHandles =null;
	...
	{
		try {
			pageHandles[pageIndex] = EMBJavaSupport.FPDFPageLoad(docHandle,pageIndex);
	...
	/*public void setCurPageHand() {
		pageHandles[currentIndex] = pageHandles[currentIndex];
	}*/
	...
	public boolean ClosePDFPage() throws toBeContinuedException, parameterException, statusException, memoryException{
		
		if (pageHandles[currentIndex] == 0){
			return true;
		}
		
		EMBJavaSupport.FPDFPageClose(pageHandles[currentIndex]);
		pageHandles[currentIndex] = 0;
	...
	private void RenderPage(int pageIndex, Bitmap bm, int startX, int startY, float xScale, float yScale, int rotate, int flags, Rectangle rect ,int pauseHandler)
	{
		try {
			int dib = EMBJavaSupport.FSBitmapCreate(bm.getWidth(), bm.getHeight(), 7, null, 0);
			EMBJavaSupport.FSBitmapFillColor(dib,0xff);
			float scaledWidth = bm.getWidth() * xScale;
			float scaledHeight = bm.getHeight() * yScale;
			EMBJavaSupport.FPDFRenderPageStart(dib,pageHandles[pageIndex],startX,startY,(int)scaledWidth,(int)scaledHeight,0,flags,rect,pauseHandler);
	...
	public Bitmap getPageBitmap(int pageIndex,int displayWidth, int displayHeight, float xScaleFactor, float yScaleFactor, int rotateFlag)
	{
		if(pageHandles[pageIndex] == 0) {
			this.InitPage(pageIndex);
		}
		...
		this.RenderPage(pageIndex,bm,0,0,xScaleFactor,yScaleFactor,rotateFlag,0,null,0);
