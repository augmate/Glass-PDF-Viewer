...
public class FoxitDoc{
	...
	private void RenderPage(int pageIndex, Bitmap bm, int startX, int startY, float xScale, float yScale, int rotate, int flags, Rectangle rect ,int pauseHandler)
	{
		try {
			...
			EMBJavaSupport.FPDFRenderPageStart(dib,pageHandles[pageIndex],startX,startY,(int)scaledWidth,(int)scaledHeight,rotate,flags,rect,pauseHandler);
	...
	public Bitmap getPageBitmap(int pageIndex,int displayWidth, int displayHeight, float xScaleFactor, float yScaleFactor, int rotateFlag)
	{
		...
		this.RenderPage(pageIndex,bm,0,0,xScaleFactor,yScaleFactor,rotateFlag,1,null,0);
		return bm;	
	}
	public int GetDoc()
	{
